...
import ClusterDemo from '../../components/ClusterDemo.astro';
...
Apple's `CLGeocoder` takes 1.3 seconds per request and is rate-limited to 50 requests per minute. For a library of thousands of photos that has to be geocoded and grouped into trips by city, that means waiting nearly 3 hours. So I replaced it with an embedded geographic database, a K-D Tree, and a chain expansion algorithm - all running offline in seconds.
...
<StatTiles
  tiles={[
    {
      label: 'Cities embedded',
      value: 167000,
      suffix: '+',
      sub: 'GeoNames cities1000, ~6 MB in the bundle',
    },
    {
      ...
      suffix: 'ms',
      decimals: 2,
      sub: 'O(log N) K-D Tree query',
    },
    {
      label: 'Full pipeline',
      value: 5,
      suffix: 's',
      sub: '8,000 photos to ~500 events',
    },
    {
      label: 'Network calls',
      value: 0,
      sub: 'Everything resolves on device',
    },
  ]}
/>
...
For a library with 8,000 photos producing 500+ event clusters, this meant 45+ seconds of geocoding with a two-phase approach: show placeholder names first, then progressively resolve the real ones. Nearly a minute of "Loading…" on screen. Unusable.

## The Solution: Embedded GeoNames + K-D Tree

### The Dataset

GeoNames is an open geographic database. I use `cities1000.txt` - 167,000 cities worldwide with population > 1,000. After trimming to only needed columns (name, latitude, longitude, country code, population), it compresses to a ~6MB TSV file embedded in the bundle.
...
Pure nearest-neighbor geocoding produces results like "Lurup, Germany" instead of "Hamburg, Germany." Lurup is a Hamburg district - its city center is closer to the photo's coordinates than Hamburg's center, so the K-D Tree returns it. But no one wants to see "Lurup" when they were in Hamburg.
...
The demo below runs the first two rules of that pipeline live in your browser on a synthetic month of photos. Every dot is a photo placed by capture time; the cards are the events the rules produce.
...
The whole pipeline, phase by phase, with the parameters each one reads:

<PhaseRail
  phases={[
    {
      title: 'Split on time and distance',
      body: 'Photos sort chronologically. A new candidate cluster starts whenever the gap to the previous photo exceeds the split threshold, or the Haversine distance jumps past 50 km.',
      params: [
        { name: 'splitTimeThreshold', value: '6h' },
        { name: 'distanceThresholdKm', value: '50' },
      ],
    },
    {
      title: 'Merge the same place',
      body: 'Adjacent clusters at the same quantized location merge across gaps up to 48 hours, so an overnight stay survives instead of shattering at every sunrise.',
      params: [{ name: 'mergeTimeThreshold', value: '48h' }],
    },
    {
      title: 'Resolve cities offline',
      body: 'Each cluster anchor resolves through the K-D Tree over the bundled GeoNames dataset, with suburb detection folding districts into their metropolis. No network, no rate limits.',
      params: [
        { name: 'cities1000.tsv', value: '167,000 cities' },
        { name: 'suburbRadiusKm', value: '20' },
        { name: 'metropolisThreshold', value: '100,000' },
      ],
    },
    {
      title: 'Merge again on the resolved name',
      body: 'Two clusters at different coordinates that both geocode to the same city name collapse into one event when they fall inside the merge window.',
      params: [{ name: 'mergeTimeThreshold', value: '48h' }],
    },
    {
      title: 'Rebuild each event by chain',
      body: 'Anchored on the city center, every library photo within 50 km is verified against the same city name, then chained through 24-hour neighbour links forward and backward.',
      params: [
        { name: 'chainGapSeconds', value: '24h' },
        { name: 'locationThresholdKm', value: '50' },
      ],
    },
    {
      title: 'Sweep the photos with no GPS',
      body: 'Once the chain fixes the date range, photos without coordinates that fall inside that window join the event. Nothing gets assigned twice.',
      params: [{ name: 'includeScreenshots', value: 'configurable' }],
    },
  ]}
/>
...
The original implementation used a fixed ±12-hour window around each event photo. Start from one photo of a 5-day Hamburg trip → only photos within 12 hours matched. 45 out of 163. Wrong.
...
Each newly-included photo extends the chain. A 5-day trip where you took at least one photo per day chains together entirely from a single seed. Switch the demo below between the two strategies to see what each one reaches from the same seed photo:

<ChainDemo />

### No-GPS Photo Sweep

After the chain establishes the trip's date range using GPS-confirmed photos, a final sweep includes photos without GPS (messaging saves, screenshots, older cameras) that fall within that window. These couldn't pass the location filter (no coordinates to verify), but they're temporally anchored to the trip. A configuration flag controls whether screenshots join that sweep.
